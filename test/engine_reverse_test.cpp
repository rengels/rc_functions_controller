...
TEST(EngineReverseTest, SetDrivingState) {
    ...
    engine.setDrivingState(EngineReverse::DrivingState::STOPPED);
    EXPECT_EQ(2, engine.gears.size()); // we set gears even in stopped state
    EXPECT_EQ(EngineReverse::DrivingState::STOPPED, engine.drivingState);
