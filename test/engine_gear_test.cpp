...
/** Unit test for EngineGear::couplingFactor() */
TEST(EngineGearTest, CouplingFactor) {
    ...
    EXPECT_NEAR(1.0f, engine.couplingFactor(), EPSILON);

    // coupled with gear 0 gives 1.0f
    engine.gearCurrent = 0;
    EXPECT_NEAR(0.0f, engine.couplingFactor(), EPSILON);
    engine.gearCurrent = 1;

    engine.gearState = EngineGear::GearState::DECOUPLED;
    EXPECT_NEAR(0.0f, engine.couplingFactor(), EPSILON);

    // for coupling is something between 0.0 and 1.0
    engine.gearState = EngineGear::GearState::COUPLING;
    engine.gearCouplingTime = 100;
    EXPECT_GT(1.0f, engine.couplingFactor());
    EXPECT_LT(0.0f, engine.couplingFactor());

    engine.gearState = EngineGear::GearState::DECOUPLING;
    engine.gearDecouplingTime = 100;
    EXPECT_GT(1.0f, engine.couplingFactor());
    EXPECT_LT(0.0f, engine.couplingFactor());

    // for 0 gears it's always 1.0f
    engine.gears.set({0.0f});
    EXPECT_NEAR(1.0f, engine.couplingFactor(), EPSILON);
}
...
TEST(EngineGearTest, DistributeEnergy) {
    ...
    engine.distributeEnergy(1.0f, 0.0f);
    EXPECT_NEAR(100, engine.energyEngine.get(), EPSILON);
    EXPECT_NEAR(200, engine.energyVehicle.get(), EPSILON);

    // -- only half-balance it
    engine.energyEngine.set(300);
    engine.energyVehicle.set(0);

    engine.distributeEnergy(0.5f, 0.0f);
    EXPECT_NEAR(200, engine.energyEngine.get(), EPSILON);
    EXPECT_NEAR(100, engine.energyVehicle.get(), EPSILON);

    // -- keep min RPM-energy
    engine.energyEngine.set(300);
    engine.energyVehicle.set(0);

    engine.distributeEnergy(0.5f, 300.0f);
...
TEST(EngineGearTest, ChooseGear) {
    ...
    EXPECT_EQ(1, engine.chooseGear(signals));

    // no throttle, upshift as long as we are over idle RPM
    engine.gearCurrent = 1;
    signals[SignalType::ST_THROTTLE] = RCSIGNAL_NEUTRAL;
    EXPECT_EQ(2, engine.chooseGear(signals));
