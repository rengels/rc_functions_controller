...
public:
    ...
    struct Meassurement {
        ...
        rcEngine::EngineSimple::EngineState state;
        ...
        float couplingFactor;   ///< coupling factor as returned by EngineGear
    ...
    void configureEngine() {
        ...
        if (config.count("rpmMax")) {
            engine->maxRPM = config["rpmMax"].as<float>();
        }
        ...
        if (engineGear) {
            ...
            if (config.count("timeCouple")) {
                engineGear->gearCouplingTime = config["timeCouple"].as<float>();
            }
            if (config.count("timeDecouple")) {
                engineGear->gearDecouplingTime = config["timeDecouple"].as<float>();
            }
    ...
    void getMeassurement(Meassurement* meas) {
        ...
        meas->gear = 0;
        meas->couplingFactor = 0;
        meas->strGearState = "--";

        if (engineGear) {
            meas->gear = engineGear->gearCurrent;
            meas->couplingFactor = engineGear->couplingFactor();
        ...
        if (engineReverse) {
            ...
            case rcEngine::EngineReverse::DrivingState::STOPPED:
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& mes : sim.meassurements) {
        ...
        cout << setprecision(1) << std::fixed <<
            ...
            setprecision(0) << std::fixed <<
            setw(5) << (mes.energyEngine) << "J, " <<

            setprecision(0) << std::fixed <<
            setw(5) << (mes.energyVehicle / 1000.0f) << "kJ, " <<

            setprecision(0) << std::fixed <<
            setw(5) << ((mes.speedVal * 60.0f) / (1.6f * M_PI)) << "rpm wheel, " <<

            setw(2) << mes.gear << " " <<

            setprecision(1) << std::fixed <<
            setw(2) << mes.couplingFactor << " " <<
