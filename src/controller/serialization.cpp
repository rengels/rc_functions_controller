...
/** Serializes ProcNeutral to a byte stream. */
SimpleOutStream& operator<<(SimpleOutStream& out,
    const ProcNeutral& proc) {
...
/** Deserializes ProcNeutral from a byte stream. */
SimpleInStream& operator>>(SimpleInStream& in,
    ProcNeutral& proc) {
...
SimpleOutStream& operator<<(SimpleOutStream& out,
    ...
    out << proc.gearCouplingFactor;
    out << proc.gearDoubleDeclutch;
    out << proc.fullGears;
    out << proc.brakePower;
    out << proc.resistance;
...
SimpleInStream& operator>>(SimpleInStream& in,
    ...
    in >> proc.gearCouplingFactor;
    in >> proc.gearDoubleDeclutch;
    in >> proc.fullGears;
    in >> proc.brakePower;
    in >> proc.resistance;
...
void ProcStorage::serializeProc(SimpleOutStream& out, const rcProc::Proc& proc) const {
    ...
    } else if (dynamic_cast<const rcProc::ProcNeutral*>(&proc)) {
        id = ProcId('N', 'e');        out << dynamic_cast<const rcProc::ProcNeutral&>(proc);
...
rcProc::Proc* ProcStorage::deserializeProc(SimpleInStream& in) {
    ...
    // -- General
        ...
        in >> *proc2;
        proc = proc2;
    } else if (id == ProcId{'N', 'e'}) {
        auto proc2 = new rcProc::ProcNeutral;
        in >> *proc2;
        proc = proc2;
